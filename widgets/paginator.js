...
//--------------------------------------------------[Paginator]
   ...
   *   调用 update 方法后触发。
   * @description
   *   “分页导航条”有两种工作模式，如果为本元素指定了 <dfn>data-target-url</dfn> 属性，即启用“静态模式”，否则启用“动态模式”。
   *   “静态模式”适用的场景为：页面由服务端代码输出，知道 totalPages 和 currentPage（通过设置 <dfn>data-total-pages</dfn> 和 <dfn>data-current-page</dfn> 属性来指定），且翻页时需要刷新页面。
   *   在“静态模式”下，不必使用任何 JS 代码来处理导航条内容。
   *   “动态模式”适用的场景为：页面加载时不知道 totalPages 和 currentPage，或当翻页时不希望刷新当前页面。
   *   在“动态模式”下，应当在合适的时间调用 update 方法来更新导航条内容，并在 turn 事件的监听器中进行后续处理。
   *   <strong>启用方式：</strong>
   *   为一个元素添加 'widget-paginator' 类，即可使该元素成为分页导航条。
   *   <strong>结构约定：</strong>
   *   当分页导航条初始化时，会自动创建其内部的元素。
   *   其中“上一页”按钮的类名为 'prev'，“下一页”按钮的类名为 'next'，页码的容器的类名为 'pages'。
   *   如果一个按钮处于禁用状态，将为其添加类名 'disabled'。
   *   表示当前页的页码元素将被添加类名 'current'。
   *   <strong>新增行为：</strong>
   *   在“静态模式”下，点击页码不会触发 turn 事件，页面将直接跳转到指定的 url 地址。
   *   在“动态模式”下，点击页码会调用 turn 方法，并触发 turn 事件，此点击事件的默认行为会被阻止。
   *   <strong>默认样式：</strong>
   *   <pre class="lang-css">
  ...
  Widget.register({
    ...
    methods: {
      turn: function(targetPage) {
        targetPage = Math.limit(targetPage, 1, this.totalPages);
      ...
      update: function(currentPage, totalPages) {
        ...
        this.targetPage = this.currentPage = currentPage = Math.limit(currentPage, 1, totalPages);
        this.totalPages = totalPages;
        // 生成页码 items。
        var endpointPages = this.endpointPages;
        var adjacentPages = this.adjacentPages;
        ...
        elements.pageNumberContainer.innerHTML = items
            ...
            .join('');
        // 触发事件。
        this.fire('update', {currentPage: currentPage, totalPages: totalPages});
        return this;
      }
    },
    initialize: function() {
      ...
      var $pageNumberContainer = document.$('<span class="pages"></span>').insertTo($paginator);
      ...
      // “静态模式”，自动更新导航条内容。
      if (targetUrl) {
        $paginator.update($paginator.currentPage, $paginator.totalPages);
      }
