...
//--------------------------------------------------[Validator]
   ...
   *   <strong>新增行为：</strong>
   *   如果一个表单域配置了“验证规则”，当其中包含的任何控件的值被用户改变时，都将对该表单域进行验证，并触发 fieldvalidate 事件，验证结束后会触发 fieldvalidated 事件。<br>如果一个表单域未能通过验证，提示信息会被注入为该表单域指定的“提示信息容器”中。<br>要手动验证某一个表单域，触发其中任一控件的 change 事件即可。
   *   当某个表单域的输入或验证状态发生变化时，“状态指示器”和“提示信息容器”的类名也会随之改变（输入中=input && 验证中=validating || 通过验证=valid || 未通过验证=invalid），可以利用此特性在各种状态下显示不同的内容。
   *   该表单的 submit 事件的默认行为将被阻止，当表单的 submit 事件发生时，会对所有已配置的“验证规则”涉及到的、且尚未验证的表单域进行验证，并触发 validate 事件，验证结束后会触发 validated 事件。<br>如果没有需要服务端验证的表单域，validated 事件将同步触发，否则 validated 事件将在所有的服务端验证结束后异步触发。<br>如果用户在可能存在的服务端验证尚未全部结束之前修改了任一控件的值，则会立即取消当前的服务端验证，并触发 validated 事件，本次验证按失败处理。
