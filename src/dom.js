...
//--------------------------------------------------[HTMLFormElement.prototype.setValidationRules]
   ...
   *     <tr><td><dfn>remote</dfn></td><td>Object</td><td>指定对该域的值进行服务端验证，包含四个属性：url、options、valueKey、validateResult。前两个属性为创建远程请求使用的 Request 的参数。valueKey 用来指定将该域的值传入服务端时使用的 key，validateResult 是处理服务端返回信息的函数，该函数可以返回“错误信息”字符串，若“错误信息”字符串为空则表示验证通过。</td></tr>
   *   </table>
