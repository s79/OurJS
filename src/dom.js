...
//==================================================[DOM 扩展 - 原型]
   ...
   *     由于对包装对象上方法的调用与对原生对象上方法的调用方式是相同的（使用 . 操作符调用），特殊情况下有误把原生对象当作包装对象误用的可能。
   *     必须以约定的方式获取元素以便对其包装。
   * 三、原型扩展
   *   方式：
...
//--------------------------------------------------[HTMLFormElement.prototype.setValidationRules]
  // 验证值是否符合规则。
  var validate = function($form, value, rules, callback) {
    var result = true;
    console.warn(value);
    if (rules.required) {
    ...
    if (result && rules.minLength) {
      result = value.length >= rules.minLength;
    }
    if (result && rules.maxLength) {
   ...
   *   {string} equals 指定该表单域的值应与哪个表单域的值一致，仅应在该表单域只包含一个文本控件时指定，并应指定一个本表单内的域的名称。省略为不限制。
   ...
   * @param {Function} options.onControlFocus 当要验证的表单域包含的控件获得焦点时执行的回调。该函数被调用时传入该表单域的名称，this 的值为本表单元素。
   * @param {Function} options.onFieldValidationStart 对每一个要验证的表单域开始验证时执行的回调。该函数被调用时传入该表单域的名称和对应的值，this 的值为本表单元素。
   * @param {Function} options.onFieldValidationFinish 对每一个要验证的表单域开始验证时执行的回调。该函数被调用时传入该表单域的名称、对应的值和验证结果，this 的值为本表单元素。
   * @returns {HTMLFormElement} 本表单。
   * @description
   *   当本表单被提交时会自动使用已配置的规则进行验证。如果有任何一条验证没有通过，则表单的提交动作会被取消。
   */
  HTMLFormElement.prototype.setValidationRules = function(fieldRules, options) {
    var onFieldValidationStart = options.onFieldValidationStart || function() {
    };
    var onFieldValidationFinish = options.onFieldValidationFinish || function() {
    };
    var $form = this.on('submit.validation', function() {
//      var counter = 0;
