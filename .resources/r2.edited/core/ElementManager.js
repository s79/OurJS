...
var ElementManager =
{
  ...
  getDocumentWidth: function() {
    ...
    if (window.ActiveXObject) {
      width = Math.max(width, ElementManager.getWindowWidth());
    }
    return width;
  },
  getDocumentHeight: function() {
    ...
    if (window.ActiveXObject) {
      height = Math.max(height, ElementManager.getWindowHeight());
    }
    return height;
  },
  ...
  getComputedStyle: function($target, styleName) {
    return document.defaultView ? document.defaultView.getComputedStyle($target, null).getPropertyValue(styleName) : $target.currentStyle[styleName.replace(/-[a-z]/g, function() {
