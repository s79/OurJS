...
(function() {
  ...
  function showAsDialog(target, callback) {
    ...
    this.dialogOffsetHandler = function() {
      ...
      this_.css({
        left: Math.max(0, Math.floor((width - selfWidth) / 2 - rect.left)),
        top: Math.max(0, Math.floor((height - selfHeight) / 2 - rect.top))
      });
    };
    ...
    this.css('visibility', 'visible').css('display', 'none').fadeIn(200, function() {
      if (callback) {
  ...
  function hideAsDialog(callback) {
    ...
    dialogs.pop();
    if (dialogs.length) {
      $.setMaskBehind(dialogs[dialogs.length - 1]);
    } else {
      $.setMaskBehind();
    }
    return this;
